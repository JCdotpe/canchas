...
class Cap4B extends CI_Controller {
	...
	public function index()
	{
		...
		if($is_ajax){
			...
			$fields = $this->encuesta_model->get_fields('CAP04B');
			//id
			...
			if ($this->encuesta_model->consulta_in_cap($id,'CAP04B')->num_rows() == 0) {
				// inserta nuevo registro
				$c_data['Postulante_Id'] = $id;
				$c_data['usr_creacion'] = $id;
				$c_data['fec_creacion'] = date('Y-m-d H:i:s');			
					if($this->encuesta_model->insert_cap('CAP04B',$c_data) > 0){
						$flag = 1;
						$msg = 'Se ha registrado satisfactoriamente el Cap. IVB';
					}
			} else {
				// actualiza
				$c_data['usr_edicion'] = $id;
				$c_data['fec_edicion'] = date('Y-m-d H:i:s');
					if($this->encuesta_model->update_cap('CAP04B',$c_data,$id) > 0){
						$flag = 1;
						$msg = 'Se ha modificado satisfactoriamente el Cap. IVB';
					}
